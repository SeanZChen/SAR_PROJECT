...
class ENet(nn.Module):
    ...
    def __init__(self, num_classes, encoder_relu=False, decoder_relu=True):
        ...
        self.classifier = nn.Conv2d(128, num_classes, 1, stride=1, padding=0, bias=False)
        self.up_8 = nn.ConvTranspose2d(num_classes,num_classes, kernel_size=2, stride=2, bias=False)
        '''
        # Stage 4 - Decoder
        ...
        self.classifier = nn.Conv2d(16, num_classes, 1, stride=1, padding=0, bias=False) 
    ...
    def forward(self, x):
        ...
        y_fp = self.classifier(x)
        y = self.up_8(y_fp)
        '''
        # Stage 4 - Decoder
        d_x = self.upsample4_0(x, max_indices2_0)
        d_x = self.regular4_1(d_x)
        d_x = self.regular4_2(d_x)
 
        #ft_map = self.classifier(d_x)

        # Stage 5 - Decoder
        d_x = self.upsample5_0(d_x, max_indices1_0)
        d_x = self.regular5_1(d_x)
        y = self.classifier(d_x)
